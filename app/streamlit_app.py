"""
streamlit_app.py
================
PURPOSE : Streamlit dashboard for the Lahore AQI Predictor.
          Loads model predictions from Hopsworks (or local CSV),
          fetches live AQI, and shows a clean interactive forecast.

HOW TO RUN:
  pip install streamlit pandas plotly requests python-dotenv hopsworks
  streamlit run streamlit_app.py

FEATURES:
  - Live AQI gauge (multi-station average)
  - 72-hour AQI forecast chart
  - Historical AQI trend (last 7 days)
  - AQI category breakdown
  - Health recommendations
  - Auto-refreshes every 30 minutes
"""

import os
from datetime import datetime, timezone

import pandas as pd
import plotly.graph_objects as go
import streamlit as st
from dotenv import load_dotenv

load_dotenv()

# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Lahore AQI Forecast",
    page_icon="🌫️",
    layout="wide",
    initial_sidebar_state="collapsed",
)

USE_HOPSWORKS     = os.environ.get("USE_HOPSWORKS", "true").lower() == "true"
HOPSWORKS_API_KEY = os.environ.get("HOPSWORKS_API_KEY", "")
PRED_GROUP_NAME   = "lahore_aqi_predictions"
FEAT_GROUP_NAME   = "lahore_aqi_features"
LOCAL_PRED_CSV    = "predictions.csv"
LOCAL_FEAT_CSV    = "lahore_features.csv"

# AQI colour thresholds
AQI_BANDS = [
    (0,   50,  "Good",                       "#00e400", "#1a1a1a"),
    (51,  100, "Moderate",                   "#ffff00", "#1a1a1a"),
    (101, 150, "Unhealthy for Sensitive",    "#ff7e00", "#ffffff"),
    (151, 200, "Unhealthy",                  "#ff0000", "#ffffff"),
    (201, 300, "Very Unhealthy",             "#8f3f97", "#ffffff"),
    (301, 500, "Hazardous",                  "#7e0023", "#ffffff"),
]

HEALTH_ADVICE = {
    "Good":                       "Air quality is satisfactory. Enjoy outdoor activities! ✅",
    "Moderate":                   "Unusually sensitive people should consider limiting prolonged outdoor activity. 🟡",
    "Unhealthy for Sensitive":    "Sensitive groups (elderly, children, those with respiratory issues) should limit outdoor exertion. 🟠",
    "Unhealthy":                  "Everyone may begin to experience health effects. Limit outdoor activity. 🔴",
    "Very Unhealthy":             "Health alert! Avoid all outdoor activity. Keep windows closed, use air purifiers. 🟣",
    "Hazardous":                  "EMERGENCY: Remain indoors. Wear N95 mask if you must go outside. Avoid all outdoor exposure. ⚫",
}


# ── Helpers ───────────────────────────────────────────────────────────────────

def classify_aqi(value: float):
    for lo, hi, label, bg, fg in AQI_BANDS:
        if lo <= value <= hi:
            return label, bg, fg
    return "Unknown", "#cccccc", "#000000"


@st.cache_data(ttl=1800)   # cache 30 minutes
def load_predictions() -> pd.DataFrame:
    if USE_HOPSWORKS and HOPSWORKS_API_KEY:
        try:
            import hopsworks
            project = hopsworks.login(api_key_value=HOPSWORKS_API_KEY)
            fs = project.get_feature_store()
            fg = fs.get_feature_group(name=PRED_GROUP_NAME, version=1)
            df = fg.read()
            df = df.sort_values("forecast_created_utc").tail(168)
            return df
        except Exception as exc:
            st.warning(f"Hopsworks predictions unavailable ({exc}). Falling back to local CSV.")

    if os.path.exists(LOCAL_PRED_CSV):
        df = pd.read_csv(LOCAL_PRED_CSV)
        df["forecast_created_utc"] = pd.to_datetime(df["forecast_created_utc"])
        return df.sort_values("forecast_created_utc").tail(168)

    return pd.DataFrame()


@st.cache_data(ttl=1800)
def load_historical() -> pd.DataFrame:
    if USE_HOPSWORKS and HOPSWORKS_API_KEY:
        try:
            import hopsworks
            project = hopsworks.login(api_key_value=HOPSWORKS_API_KEY)
            fs = project.get_feature_store()
            fg = fs.get_feature_group(name=FEAT_GROUP_NAME, version=1)
            df = fg.read()
            df = df.sort_values("timestamp").tail(7 * 24)
            return df
        except Exception:
            pass

    if os.path.exists(LOCAL_FEAT_CSV):
        df = pd.read_csv(LOCAL_FEAT_CSV)
        df["timestamp"] = pd.to_datetime(df["timestamp"])
        return df.sort_values("timestamp").tail(7 * 24)

    return pd.DataFrame()


@st.cache_data(ttl=1800)
def fetch_live_aqi() -> dict:
    try:
        from live_aqi_client import fetch_lahore_aqi_average
        return fetch_lahore_aqi_average()
    except Exception as exc:
        return {"aqi": None, "error": str(exc), "stations_used": 0}


# ── Gauge chart ───────────────────────────────────────────────────────────────

def make_aqi_gauge(value: float, title: str = "AQI") -> go.Figure:
    label, bg, _ = classify_aqi(value)
    fig = go.Figure(go.Indicator(
        mode="gauge+number+delta",
        value=value,
        title={"text": title, "font": {"size": 18}},
        delta={"reference": 100, "increasing": {"color": "#ff4444"}, "decreasing": {"color": "#44bb44"}},
        gauge={
            "axis": {"range": [0, 300], "tickwidth": 1},
            "bar": {"color": bg, "thickness": 0.3},
           "steps": [
    {"range": [0,   50],  "color": "rgba(0,228,0,0.2)"},
    {"range": [51,  100], "color": "rgba(255,255,0,0.2)"},
    {"range": [101, 150], "color": "rgba(255,126,0,0.2)"},
    {"range": [151, 200], "color": "rgba(255,0,0,0.2)"},
    {"range": [201, 300], "color": "rgba(143,63,151,0.2)"},
],
            "threshold": {
                "line": {"color": bg, "width": 4},
                "thickness": 0.75,
                "value": value,
            },
        },
        number={"font": {"size": 48, "color": bg}},
    ))
    fig.update_layout(
        height=280,
        margin=dict(l=20, r=20, t=40, b=10),
        paper_bgcolor="rgba(0,0,0,0)",
        font={"color": "#ffffff"},
    )
    return fig


# ── Forecast chart ─────────────────────────────────────────────────────────────

def make_forecast_chart(df_pred: pd.DataFrame, df_hist: pd.DataFrame) -> go.Figure:
    fig = go.Figure()

    # Historical line
    if not df_hist.empty and "us_aqi" in df_hist.columns:
        fig.add_trace(go.Scatter(
            x=df_hist["timestamp"],
            y=df_hist["us_aqi"],
            name="Historical AQI",
            line=dict(color="#4a9eff", width=2),
            mode="lines",
        ))

    # Forecast points
    if not df_pred.empty:
        latest = df_pred.iloc[-1]
        now = pd.to_datetime(latest["forecast_created_utc"])

        future_times = [
            now + pd.Timedelta(hours=24),
            now + pd.Timedelta(hours=48),
            now + pd.Timedelta(hours=72),
        ]
        future_vals = [
            latest.get("pred_aqi_24h", 0),
            latest.get("pred_aqi_48h", 0),
            latest.get("pred_aqi_72h", 0),
        ]
        colours = [classify_aqi(v)[1] for v in future_vals]

        fig.add_trace(go.Scatter(
            x=future_times,
            y=future_vals,
            name="Forecast AQI",
            mode="lines+markers",
            line=dict(color="#ff9f43", width=2, dash="dash"),
            marker=dict(size=12, color=colours, line=dict(width=2, color="#ffffff")),
        ))

        # Vertical line at "now"
        fig.add_vline(
            x=now.timestamp() * 1000,
            line_dash="dot",
            line_color="rgba(255,255,255,0.4)",
            annotation_text="Now",
            annotation_position="top left",
        )

    # AQI threshold lines
    for threshold, label, colour in [(150, "Unhealthy", "#ff0000"), (200, "Very Unhealthy", "#8f3f97")]:
        fig.add_hline(
            y=threshold,
            line_dash="dash",
            line_color=colour,
            line_width=1,
            annotation_text=label,
            annotation_position="right",
        )

    fig.update_layout(
        title="AQI — Historical (7 days) + 72-Hour Forecast",
        xaxis_title="Time (UTC)",
        yaxis_title="US AQI",
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(255,255,255,0.04)",
        font={"color": "#ffffff"},
        legend=dict(bgcolor="rgba(0,0,0,0)", orientation="h", y=-0.15),
        height=380,
        margin=dict(l=40, r=40, t=50, b=60),
        yaxis=dict(gridcolor="rgba(255,255,255,0.08)"),
        xaxis=dict(gridcolor="rgba(255,255,255,0.08)"),
    )
    return fig


# ── Prediction history chart ──────────────────────────────────────────────────

def make_prediction_history_chart(df_pred: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    if df_pred.empty:
        return fig

    fig.add_trace(go.Scatter(
        x=df_pred["forecast_created_utc"],
        y=df_pred["pred_aqi_24h"],
        name="24h Prediction",
        line=dict(color="#ff9f43", width=2),
    ))
    fig.add_trace(go.Scatter(
        x=df_pred["forecast_created_utc"],
        y=df_pred.get("live_aqi", pd.Series()),
        name="Live AQI at time of prediction",
        line=dict(color="#4a9eff", width=1, dash="dot"),
    ))
    fig.update_layout(
        title="24h Forecast Accuracy — Predictions Over Time",
        xaxis_title="Prediction made at (UTC)",
        yaxis_title="AQI",
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(255,255,255,0.04)",
        font={"color": "#ffffff"},
        height=280,
        margin=dict(l=40, r=20, t=50, b=40),
        yaxis=dict(gridcolor="rgba(255,255,255,0.08)"),
        xaxis=dict(gridcolor="rgba(255,255,255,0.08)"),
    )
    return fig


# ── CSS styling ───────────────────────────────────────────────────────────────

st.markdown("""
<style>
body, .stApp { background-color: #0d1117; color: #c9d1d9; }
.metric-card {
    background: linear-gradient(135deg, #161b22 0%, #21262d 100%);
    border: 1px solid #30363d;
    border-radius: 12px;
    padding: 20px 24px;
    margin-bottom: 12px;
}
.aqi-badge {
    display: inline-block;
    padding: 6px 14px;
    border-radius: 20px;
    font-weight: 700;
    font-size: 0.85rem;
    letter-spacing: 0.05em;
    margin-top: 4px;
}
.section-header {
    font-size: 0.7rem;
    letter-spacing: 0.15em;
    text-transform: uppercase;
    color: #8b949e;
    margin-bottom: 4px;
}
h1 { color: #e6edf3 !important; }
.stPlotlyChart { border-radius: 12px; overflow: hidden; }
</style>
""", unsafe_allow_html=True)


# ── Main dashboard ─────────────────────────────────────────────────────────────

def main():
    # Header
    col_title, col_refresh = st.columns([4, 1])
    with col_title:
        st.markdown("# 🌫️ Lahore AQI Forecast")
        st.markdown(
            f"<span style='color:#8b949e; font-size:0.85rem;'>Last updated: "
            f"{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}</span>",
            unsafe_allow_html=True,
        )
    with col_refresh:
        if st.button("🔄 Refresh", use_container_width=True):
            st.cache_data.clear()
            st.rerun()

    st.divider()

    # Load data
    with st.spinner("Loading data…"):
        live_data  = fetch_live_aqi()
        df_pred    = load_predictions()
        df_hist    = load_historical()

    live_aqi = live_data.get("aqi") or 0.0

    # ── Row 1: Current AQI + gauges ───────────────────────────────────────────
    col1, col2, col3, col4 = st.columns(4)

    live_label, live_bg, live_fg = classify_aqi(live_aqi)
    with col1:
        st.markdown('<div class="section-header">Live City AQI</div>', unsafe_allow_html=True)
        st.markdown(
            f"<div style='font-size:3.5rem; font-weight:800; color:{live_bg};'>{live_aqi:.0f}</div>"
            f"<div class='aqi-badge' style='background:{live_bg}; color:{live_fg};'>{live_label}</div>",
            unsafe_allow_html=True,
        )
        stations_used = live_data.get("stations_used", 0)
        st.markdown(
            f"<div style='color:#8b949e; font-size:0.75rem; margin-top:8px;'>"
            f"Averaged from {stations_used} station(s)</div>",
            unsafe_allow_html=True,
        )

    if not df_pred.empty:
        latest = df_pred.iloc[-1]
        for col, hours, key in [(col2, 24, "pred_aqi_24h"),
                                 (col3, 48, "pred_aqi_48h"),
                                 (col4, 72, "pred_aqi_72h")]:
            val = float(latest.get(key, 0))
            label, bg, fg = classify_aqi(val)
            with col:
                st.plotly_chart(make_aqi_gauge(val, f"{hours}h Forecast"), use_container_width=True)
    else:
        for col, hours in [(col2, 24), (col3, 48), (col4, 72)]:
            with col:
                st.info(f"No {hours}h forecast available yet.\nRun inference_pipeline.py first.")

    # ── Health advice ─────────────────────────────────────────────────────────
    advice = HEALTH_ADVICE.get(live_label, "")
    if advice:
        colour_map = {
            "Good": "success", "Moderate": "warning",
            "Unhealthy for Sensitive": "warning", "Unhealthy": "error",
            "Very Unhealthy": "error", "Hazardous": "error",
        }
        level = colour_map.get(live_label, "info")
        getattr(st, level)(f"**Health Advice:** {advice}")

    # ── Forecast chart ────────────────────────────────────────────────────────
    st.plotly_chart(make_forecast_chart(df_pred, df_hist), use_container_width=True)

    # ── Row 2: Station breakdown + prediction history ─────────────────────────
    col_left, col_right = st.columns(2)

    with col_left:
        st.markdown("### 📡 Station Readings")
        station_readings = live_data.get("station_readings", [])
        if station_readings:
            for sr in station_readings:
                s_label, s_bg, s_fg = classify_aqi(float(sr["aqi"]))
                st.markdown(
                    f"<div class='metric-card' style='border-left: 4px solid {s_bg};'>"
                    f"<div class='section-header'>{sr['station']}</div>"
                    f"<span style='font-size:1.8rem; font-weight:700; color:{s_bg};'>{sr['aqi']:.0f}</span>"
                    f"&nbsp;&nbsp;<span class='aqi-badge' style='background:{s_bg}; color:{s_fg};'>{s_label}</span>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
        else:
            st.info("Live station data unavailable. Set AQICN_TOKEN in your .env file.")

    with col_right:
        st.markdown("### 📈 Prediction History")
        if not df_pred.empty:
            st.plotly_chart(make_prediction_history_chart(df_pred), use_container_width=True)
        else:
            st.info("No prediction history yet. Predictions will appear here after inference_pipeline.py runs.")

    # ── AQI reference table ───────────────────────────────────────────────────
    with st.expander("📋 AQI Reference Guide"):
        for lo, hi, label, bg, fg in AQI_BANDS:
            advice_text = HEALTH_ADVICE.get(label, "")
            st.markdown(
                f"<div style='display:flex; align-items:center; gap:16px; padding:8px 0; "
                f"border-bottom: 1px solid #21262d;'>"
                f"<div class='aqi-badge' style='background:{bg}; color:{fg}; min-width:60px; text-align:center;'>"
                f"{lo}–{hi}</div>"
                f"<div><strong style='color:{bg};'>{label}</strong><br>"
                f"<span style='font-size:0.82rem; color:#8b949e;'>{advice_text}</span></div>"
                f"</div>",
                unsafe_allow_html=True,
            )

    # ── Footer ────────────────────────────────────────────────────────────────
    st.divider()
    st.markdown(
        "<div style='text-align:center; color:#8b949e; font-size:0.75rem;'>"
        "Lahore AQI Predictor • Data: AQICN + Open-Meteo • "
        "Models: Random Forest / LSTM • Refreshes every 30 min"
        "</div>",
        unsafe_allow_html=True,
    )

    # Auto-refresh every 30 minutes
    st.markdown(
        "<script>setTimeout(() => window.location.reload(), 1800000);</script>",
        unsafe_allow_html=True,
    )


if __name__ == "__main__":
    main()